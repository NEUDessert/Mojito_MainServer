...
//        List<MedicalRecordEntity> aliasAndName = userDAO.getCritical(phoneNumber);
//        // 创建得到当前时间的对象 time
...
//        userDAO.addAlert(alertEntity);
    ...
    public int sendCritical(AlertEntity alertEntity) throws ParseException {
        // 获取发生心脏异常用户手机号，每个用户用手机号注册，手机号每个用户都是唯一的
        String phoneNumber = alertEntity.getPhoneNumber();
        // 获取发生心脏异常的用户的名字、监护人在JPush推送平台的注册的alias
        List<MedicalRecordEntity> aliasAndName = userDAO.getCritical(phoneNumber);
        // 创建得到当前时间的对象 time
        GetTime time = GetTime.singleGetCurrentTime();
...
//            JPushServerMain.send(aliasAndName.get(i)[0], "您的亲人" + aliasAndName.get(i)[1] + time.getTime() + "突发心脏异常");
//        }
        if(aliasAndName.size()!=0) {
            if (aliasAndName.get(0).getContactsNumber1()!=null)
                JPushServerMain.send(aliasAndName.get(0).getContactsNumber1(), "您的被监护人" + aliasAndName.get(0).getContactsName1() + time.getTime() + "突发心脏异常");
            if (aliasAndName.get(0).getContactsNumber2()!=null)
                JPushServerMain.send(aliasAndName.get(0).getContactsNumber2(), "您的被监护人" + aliasAndName.get(0).getContactsName2() + time.getTime() + "突发心脏异常");
        }

        // 从数据库中拿到所有的AED的信息
        List<DeviceEntity> list = userDAO.getAllAED();
        // 将获得的String型经纬度转换为Double类型
        double recvLocaX = Double.parseDouble(alertEntity.getAlertLocX());
        double recvlocaY = Double.parseDouble(alertEntity.getAlertLocY());
        double distance = -1; // 设置发生心脏异常用户与最近的AED的距离，初始值为 -1
        int  deviceId =-1 ; // 离发生心脏异常用户最近的AED的 设备ID deviceId
        // 计算离发生心脏异常用户最近的AED
        for (int j = 0; j < list.size(); j++) {
            double locaX = Double.parseDouble(list.get(j).getLocX());
            double locaY = Double.parseDouble(list.get(j).getLocY());
            double temp = Math.sqrt(Math.pow((recvLocaX-locaX),2)+Math.pow((recvlocaY-locaY),2))*1000;
            // 判断此AED（AED-1）离发生心脏异常用户的距离与此前最小的值（AED-X）的大小，如果AED-1<AED-X， 则用AED-1的值代替AED-X的值成为最小值
            if(temp<distance){
                distance = temp;
                deviceId = list.get(j).getDeviceId();
            }
            // 当distance=-1时，即第一次计算距离时，执行此操作
            else if(distance==-1){
                distance = temp;
    ...
    public double[] userGetLocation(String phoneNumber){
        return userDAO.userGetLocation(phoneNumber);
    }
